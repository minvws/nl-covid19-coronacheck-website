const html = `
    <div class="footer-links">
        <div class="footer-links-col ___desktop-50">
            <div class="footer-logos-info">
                <h2 class="screen-reader-text">Colophon</h2>
                The CoronaCheck and CoronaCheck Scanner apps have been developed by the Ministry of Public Health,
                Welfare and Sport. The apps would not have been possible without the support of dozens of
                specialists, hands-on experts and a large open-source community of citizens, who all lent a hand
                and helped to test the app during its development.
            </div>
        </div>
        <div class="footer-links-col ___desktop-50">
            <h3> Do you have any questions about CoronaCheck?<br>
            Call <a href="tel:08001421">0800-1421</a> (Free)
            </h3>
            <p><strong>Opening hours</strong><br> Monday till Sunday 08.00 a.m. - 08.00 p.m. </p>
            <p><strong>Or send an e-mail to:</strong><br>
            <a href="mailto:helpdesk@coronacheck.nl" target="_blank"> helpdesk@coronacheck.nl </a>
            </p>
        </div>
    </div>
`;

export default html
